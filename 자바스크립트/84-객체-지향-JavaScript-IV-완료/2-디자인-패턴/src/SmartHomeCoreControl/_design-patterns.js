// 싱글톤 패턴 (Singleton)
{
  ...
  const hub3 = CentralHub.get()
  const hub4 = CentralHub.get()
  const hub5 = new CentralHub()
...
// 팩토리 패턴 (Factory)
{
  ...
  const myAircon = DeviceFactory.create('aircon')
...
// 전략 패턴 (Strategy)
{
  ...
  class MaxStrategy {
    execute(name) { return `${name} 최고 성능 모드 🔋`}
  }
  ...
  // '최고 파워' 모드로 전략 설정 (런타임 중에 변경 가능)
  deviceRunner.setStrategy(new MaxStrategy())
  deviceRunner.run('세탁기') // 세탁기 가동 (최고 모드로)
