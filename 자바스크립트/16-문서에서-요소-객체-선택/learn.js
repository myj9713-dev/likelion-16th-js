...
// 아주 오래 전 웹 브라우저 세상에서 사용했던 오래된 방법 (⚠️비표준 : Microsoft사의 IE6 브라우저)
...
// * JavaScript 엔진 
//   - *.js 파일 처리 -> DOM에 접근/조작
//   - *.js 파일 처리하는 동안 HTML 파서는 대기 상태가 됨. (일을 안하고 대기 중...)
...
// document.getElementById 함수 흉내내기
// function getElementById(idName) {
  // let isFoundElement = false
  // 문서에서 요소(객체) 찾기
  // 요소를 찾으면 찾은 요소(객체) 반환
  // if (isFoundElement) {
    // return {} // 찾은 문서 요소(Element 객체)
  // }
  // 요소를 못찾으면 못찾았다는 것을 의미하는 null 반환
//   else {
//     return null
//   }
// }
...
// if (!chapterElement === null) { // (!chapterElement)도 가능
//   console.warn('#chapter 요소가 문서에 없습니다.')
// }
// if (!chapterElement) console.warn('#chapter 요소가 문서에 없습니다.') // 한줄로 작성
// chapterElement || console.warn('#chapter 요소가 문서에 없습니다.') // ||(or연산자)는 true을 찾아서 반환
...
//   console.warn('.sr-only 선택자로 요소를 찾을 수 없습니다.')
// }
...
// 반복되는 코드르 재사용하기 위해 함수를 작성하기로 했다.
// 함수(기능)에 걸맞는 이름을 작성하기로 했다.
// 어떤 기능: 문서에 요소가 존재하는지 확인한 후, 존재하지 않을 경우 콘솔에 경고하기로 했다.
...
const liPElement = document.querySelector('li')
// console.log(liPElement)
...
checkElementWarn(firstMusician, 'li:first-child')
...
const allMusicians = musicianList.querySelectorAll('li')
...
console.log(yohan) // <li> (HTML Code) -> HTMLListitemElement (DOM Object)
...
//     1-2. 선택자로 찾기
villainGhost = exorcistCharacters.querySelector('li:nth-child(3)')
//     1-3. 클래스 이름으로 찾기
