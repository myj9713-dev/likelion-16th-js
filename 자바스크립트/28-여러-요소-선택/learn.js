...
// NodeList (배열과 유사한 객체, ⚠️배열 객체는 아님)
...
// 여기에 코드를 작성하세요.
const allParagtaph = document.querySelectorAll('main p')
console.log(allParagtaph)

// 배열 바꾸기 (NodeList -> Array.from() -> 새로운 Array 생성)
const allParagraphArray = Array.from(allParagtaph)
console.log(Array.isArray(allParagtaph)) // false
console.log(typeof allParagraphArray) // object
// 배열인지 아닌지 정확히 감지하는 메서드
// Array.isArray()
console.log(Array.isArray(allParagraphArray)) // true false
...
// 그래? 그려면 반복문을 사용해 HTMLCollection 객체를 반복하자.
// - ✅ while (index가 필요해!)
// - ✅ do...while
