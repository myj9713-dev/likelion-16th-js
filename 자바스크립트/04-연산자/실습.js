...
// 거듭제곱 연산자 (**)
console.log(2 * 2 * 2 * 2 * 2, 2 ** 5) // 2의 5승
...
console.log(count++) // 메모리에서는 1 증가된 상태로 업데이트됐지만 반환값은 업데이트 이전 값을 내놓음
console.log(count) // 이 단계에서 업데이트된 count값을 출력

// 선 증가 (Pre-increment)
console.log(++count) // 메모리에 있는 count 값을 먼저 1 증가시키고, 증가된 값을 바로 출력
console.log(count) // 위에가 바뀐 상태라서 여전히 5
...
// 숫자 + 문자열
result2 = myFavoriteNumber + greetingMessage
// result = 9 + '안녕!'
// result = '9' + '안녕!'
// result = '9안녕!'
console.log(result, typeof result)  
...
console.log(memberName < partnerName) // 김 < 이 (사전순)
console.log('박아영' < '황현주')
console.log('황현주' < '최수인')
...
// 전위: 먼저 증가/감소 후 사용
console.log(++count2)  // 6 (count는 6이 됨)
console.log(count2)    // 6

count2 = 5  // 초기화

// 후위: 먼저 사용 후 증가/감소
console.log(count2++)  // 5 (출력 후 count가 6이 됨)
console.log(count2)    // 6